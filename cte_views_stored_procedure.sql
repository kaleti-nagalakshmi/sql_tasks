...
-- 22
delimiter //
create procedure employees_id(in emp_id int ,out emp_salary int)
begin 
...
delimiter ;
set @salary = 0;
call employees_id(1,@salary);
select @salary ;

-- 23 
drop procedure if exists department_name;
delimiter //
create procedure department_name(in dept_name varchar(100),emp_salary int )
begin 
select * from employees
where department = dept_name and salary > emp_salary;

end //
delimiter ;
call department_name("HR",50000);

-- 24 
delimiter //
create procedure avg_salry(in dept_name varchar(100),
...
department,salary,dept_avg_salary,dept_rank,difference_from_avg,previous_salary from emp_data
where salary > dept_avg_salary and dept_rank in (1,2)
